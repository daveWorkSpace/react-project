...
export default {
			...
			history.listen(location => {
				...
				switch (pathname) {
					...
					case '/Materiel/Dispatcher/Detail':
						...
						break;
					// 物品模板
					case '/Materiel/Dispatcher/Create':
						const citySpelling = aoaoBossTools.getAllAreaFromPermission('city_list');
						dispatch({
							type: 'createBaseItemListE',
							payload: {
								account_id: accountId,
								city_spelling: citySpelling[0]['city_spelling'],
							}
						});
						break;
				}
	...
	effects: {
		...
		*createNewOrderE({ payload }, { call, put }) {
			...
			if (result.ok) {
				message.sucess(`创建成功,单号为${result.order_id}`, 5);
				location.href = '/#/Materiel/Purchase/Create';
			}
		},
