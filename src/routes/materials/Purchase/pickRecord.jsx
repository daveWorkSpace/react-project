...
class PickRecord extends Component {
	constructor(props) {
		super(props);
		this.state = {
			total: props.materials.pickOrderList._meta.result_count || 0,
			cityList: [],
			page: 1,													// 默认页码
			current: 1,												// 默认的高亮页码
			pageSize: 30,
			...
			columns: [{
				...
				render: (text, record) => {
					return (
						<span>
							<span><Link to={`Materiel/Purchase/Detail?id=${record._id}`}
													className="mgl8 systemTextColor pointer">查看详情</Link></span>
							{
								new Permission().createErrorOrder(record.state) == true ?
									<span><Link to={`Materiel/Purchase/ErrorOrder?id=${record._id}`}
															className="mgl8 systemTextColor pointer">报错</Link></span> : ''
							}
							{
								new Permission().agreeMaterialsApproval(record.flow_state) == true ?
									<span>
			...
			dataSource: props.materials.pickOrderList.data || [],
			searchValue: '',
		}
	}
	...
	componentWillReceiveProps(nextProps) {
		this.setState({
			total: nextProps.materials.pickOrderList._meta.result_count || 0,
			dataSource: nextProps.materials.pickOrderList.data || [],
		})
	}
