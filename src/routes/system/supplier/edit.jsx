...
class Edit extends Component {
	constructor(props) {
		...
		this.state = {
			...
			initialSupplierValue: props.system.supplierDetail,
			columns: [{
				title: '城市',
			...
			dataSource: props.system.supplierDetail.biz_district_info_list || [],
		}
	}

	componentWillReceiveProps(nextProps) {
		this.setState({
			initialSupplierValue: nextProps.system.supplierDetail,
			dataSource: nextProps.system.supplierDetail.biz_district_info_list || []
		})
	}
	...
	handleSubmits = () => {
		...
		form.validateFields((err, values) => {
			if (err) {
				...
				// });
				dispatch({
					type: 'system/setAreaBundleListR',
					payload: addedArea,
				})
			}
		});
	...
	saveAddArea = () => {
		...
		const id = this.state.initialSupplierValue._id;
		const state = this.state.initialSupplierValue.state;
		form.validateFields((err, values) => {
			if (err) {
				return;
			} else {
						...
						<Col sm={6}>
							<FormItem {...formItemLayout} label={`供应商名称`}>
								{getFieldDecorator(`supplier_name`, {
									...
									initialValue: this.state.initialSupplierValue.supplier_name,
								})(
